...
from yasearch.configuring.inject import Injector
from yasearch.etc.pattern import singleton
from yasearch.modeling.mask import GRANTED_MODEL_NAMES
from yasearch.processing.mask import GRANTED_PROCESSOR_NAMES
...
class ILoader:
    ...
    def ignite_component(
        self,
        component: str,
        wtf: Union[torch.nn.Module, Union[str, Path]],
        ...
        if component.lower() in GRANTED_MODEL_NAMES:
            ...
            raise ValueError(
                f"Unexpected argument {str(wtf)} propagated inside `Loader`"
            )
